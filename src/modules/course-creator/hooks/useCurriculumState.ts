...
import type { Module, Lesson, LessonType, LessonStatus, StatusIconInfo } from '../types';
...
export function useCurriculumState() {
  ...
  const updateLesson = (moduleId: number, lessonId: string, updates: Partial<Lesson>) => {
    setModules((prev) =>
      prev.map((mod) =>
        mod.id === moduleId
          ? {
  ...
  const setLessonStatus = (moduleId: number, lessonId: string, status: LessonStatus) => {
    updateLesson(moduleId, lessonId, { status });
  };
